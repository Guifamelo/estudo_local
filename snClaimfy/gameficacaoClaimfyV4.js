...
function run() {
    var grOposemanal = new GlideRecord('x_jam_special_oppo_oportunidades_claims');
    grOposemanal.addEncodedQuery('^sys_created_on>javascript:gs.endOfLastWeek()');
    grOposemanal.query();

    var userSemanal = [];
    while (grOposemanal.next()) {
        userSemanal.push(grOposemanal.getValue('opened_by'));


        for (i in userSemanal) {


            var grUser = new GlideRecord('sys_user');
            grUser.get(userSemanal[i]);

            if (grUser) {
                var company = grUser.getDisplayValue('company')
                gs.info(company)
            }
