function run() {	
	...
	// Semanalmente o script varre os Originadores de Oportunidades Claimfy para atualizar sua Classificação e Comissão na tabela de Intermediários
    ...
    for (i in originadores) {
        var gr = new GlideRecord('x_jam_special_oppo_oportunidades_claims');      //BUSCA O TOTAL DE OPOCS DO ORIGINADOR NA PLATAFORMA
        ...
        while (gr.next()) {
            ...
            //	pegar a data de abertura. se menor q uma semana, ooouuu melhor fazer outro glideqrecord la em baixo..  << CANCELAR TEXTO..
        }

        var qtdeOpo = opocs.length;          //TOTAL DE OPOC NA PLATAFORMA
        var qtdInv = investidos.length;      //TOTAL OPOCS INVESTIDAS
        var classificacaoOpo = '';
        var classificacaoInv = '';
        var classificacaoFreq = '';
        var classificacaoLead = '';

        //		Performance   (50% qtde de opocs, 30% Opoc Investidas, 10% Freq Semanal, 10% Leadtime)
        ...
        //  PEGA AS OPORTUNIDADES DA SEMANA PASSADA ABERTA PELO ORIGINADOR
		
        var opocSemana = new GlideRecord('x_jam_special_oppo_oportunidades_claims');
        opocSemana.addEncodedQuery('^u_originador='+originadores[i]+'^opened_at>javascript:gs.endOfLastWeek()');
        opocSemana.query();
        ...
        var opoc = new GlideRecord('x_jam_special_oppo_oportunidades_claims');    // PEGA AS OPOCS DO ORIGINADOR PARA AS TOPC DESSAS OPOCS E VER O LEAD 
